...
import numpy as np
from utils import pyramid_roi_generate, expand_roi, get_patch,get_transform, nms
...
class TextDetector:
    def __init__(self, model_path, transform, ratios=[0.5,1,2,4]):
        """
        model_path: the cnn model  
        transform: preprocessing transform used during training
        ratios: the scales applied during the generation of MSER pyramid 
        """
    ...
    def get_tensor_from_rois(self, rois, img):
        img_crops = []
        for roi in rois:
            patch = get_transform(get_patch(img, roi), self.transform)
    ...
    def split_error_connected_component(self, roi, img):
        # Pad the original roi and reize the patch to 32 x X
    ...
    def detection(self, img):
        rois_raw = pyramid_roi_generate(img, self.ratios)
        rois = expand_roi(rois_raw, img.shape[1], img.shape[0])
        if len(rois) == 0:
            return None, None, None
        input_tensor = self.get_tensor_from_rois(rois, img)
        results = self.model(input_tensor)
        ...
        for i, roi in enumerate(rois):
            if (label_raw[i] > 0) and (score_raw[i] > 0):
                ...
                if aspect_ratio > 0.7:
                    rois_split, score_split, label_split = self.split_error_connected_component(roi, img)
